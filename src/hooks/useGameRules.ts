...
import {removeFigure, setGameStarted, setGameWon} from "../redux/gameSlice";
import {Colors, FigureData, Figures, TCellsFigure} from "../types";
...
export const useGameRules = () => {
    ...
    const moveOrEat = (
        ...
        if (figureOnCell && figureOnCell.color !== figure.color) eatFigure(cellsFigure, figureOnCell);
        moveOn(figure, x, y);
    }
    ...
    const getAvailableCells = (
        ...
        const isEatableCell = (x: number, y: number): boolean => {
            return !!(cellsFigure[`${x}-${y}`] && figure.color !== cellsFigure[`${x}-${y}`]?.color);
    ...
    const checkIsKingInCheck = (
        ...
        dangerousCells: MutableRefObject<{ white: { [key: string]: boolean }; black: { [key: string]: boolean } }>,
        setIsKingInCheck: React.Dispatch<React.SetStateAction<boolean>>,
